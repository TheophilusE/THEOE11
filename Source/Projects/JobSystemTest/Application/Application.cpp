...
void App::InitTouchInput()
{
    ...
    XMLFile* layout = cache->GetResource<XMLFile>("UI/ScreenJoystick_Apps.xml");
...
void App::SetLogoVisible(bool enable)
{
    if (logoSprite_)
...
void App::SetWindowTitleAndIcon()
{
    ...
    graphics->SetWindowTitle("rbfx App");
}
        ...
        if (key == KEY_SELECT && touchEnabled_)
        {
            ...
            if (screenJoystickSettingsIndex_ == M_MAX_UNSIGNED)
            {
                ...
                screenJoystickSettingsIndex_ = (unsigned)input->AddScreenJoystick(
                    cache->GetResource<XMLFile>("UI/ScreenJoystickSettings_Apps.xml"),
...
{
    // Move the camera by touch, if the camera node is initialized by descendant App class
