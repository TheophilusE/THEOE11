...
Button* PopupMessageWindow::CreateButton(const eastl::string& text, int width, IntVector2 position)
{
...
Text* PopupMessageWindow::CreateLabel(const eastl::string& text, int fontSize)
{
    auto* cache = GetSubsystem<ResourceCache>();
    auto* font = cache->GetResource<Font>(APPLICATION_FONT);
