...
struct Resolution
{
    ...
    static Resolution FromString(const eastl::string& s)
    {
    ...
    eastl::string ToString(bool with_rate = false) const
    {
        if (!with_rate)
...
typedef eastl::vector<Resolution> ResolutionVector;
...
eastl::vector<int> GetFullscreenRefreshRates(int monitor)
{
    eastl::vector<int> rates;
    if (monitor < 0)
        return rates;
...
void SettingsWindow::CreateControlsTab()
{
    ...
    for (auto it = names.begin(); it != names.end(); ++it)
    {
        ...
        button->SetStyleAuto();
        button->SetName("Controls_" + fmt::format("{}", actionId));
...
void SettingsWindow::ApplyVideoOptions()
{
    ...
    graphics->SetMode(res.width, res.height, fullscreen == 2, fullscreen == 1, true, false,
                      !!opt_vsync_->GetOptionValue(), false, 0, opt_monitor_->GetOptionIndex(), res.refreshrate, false);
...
void SettingsWindow::ApplyGraphicsOptions()
{
    ...
    GetSubsystem<ConfigManager>()->Set("graphics", "MaxOccluderTriangles",
                                       opt_occlusion_->GetOptionIndex() > 0 ? 5000 : 0);
...
void SettingsWindow::HandleOptionChanged(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        eastl::string type = option->GetVar("AudioType").GetString();
    ...
    if (option->HasTag("joystick_as_first"))
    {
        ...
        GetSubsystem<ConfigManager>()->Set("joystick", "JoystickAsFirstController",
                                           joystick_as_first_->GetOptionValue());
    }
