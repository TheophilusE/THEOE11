...
void UIOption::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE("Option Name", GetOptionName, SetOptionName, eastl::string, "", AM_DEFAULT);
}
...
void UIOption::SetOptionName(const eastl::string& name)
{
    option_name_ = name;
    if (label_)
...
void UIBoolOption::SetOptionLabel(bool state, const eastl::string& text)
{
...
eastl::string UIBoolOption::GetOptionLabel(bool state) const
{
    auto label = GetChildDynamicCast<Text>(state ? "BtnOff" : "BtnOn", true);
    if (label)
...
void UIMultiOption::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE("Option Strings", GetOptionsAttr, SetOptionsAttr, VariantVector,
...
eastl::string UIMultiOption::GetValue() const
{
    if (strings_.empty())
        return "";
...
void UITabPanel::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE("Tabs", GetTabsAttribute, SetTabsAttribute, VariantVector,
                                    Variant::emptyVariantVector, AM_DEFAULT)
...
ListView* UITabPanel::AddTab(const eastl::string& name)
{
...
void UITabPanel::RemoveTab(const eastl::string& name)
{
    auto index = -1;
    // find the tab index
