...
CheckBox* NewGameSettingsWindow::CreateCheckbox(const eastl::string& label)
{
...
Button* NewGameSettingsWindow::CreateButton(UIElement* parent, const eastl::string&text, int width, IntVector2 position)
{
    auto* cache = GetSubsystem<ResourceCache>();
    auto* font = cache->GetResource<Font>(APPLICATION_FONT);
    ...
    for (auto it = maps.begin(); it != maps.end(); ++it)
    {
        ...
        SubscribeToEvent(button, E_RELEASED,
                         [&](StringHash eventType, VariantMap& eventData)
                         {
                             ...
                             data["StartServer"] = startServer_->IsChecked();
#endif
                             data["ConnectServer"] = connectServer_->IsChecked() && !startServer_->IsChecked()
                                                         ? "127.0.0.1"
                                                         : "";
                             SendEvent(E_SET_LEVEL, data);
                         });
