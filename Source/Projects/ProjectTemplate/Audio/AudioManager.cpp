...
void AudioManager::Init() { SubscribeToEvents(); }
...
void AudioManager::RegisterAudio(AudioDefs::SOUND_EFFECTS effect, eastl::string file) { soundEffects_[effect] = file; }

void AudioManager::RegisterAudio(AudioDefs::MUSIC music, eastl::string file) { music_[music] = file; }

void AudioManager::RegisterAudio(AudioDefs::AMBIENT_SOUNDS ambient, eastl::string file)
{
...
void AudioManager::HandlePlaySound(StringHash eventType, VariantMap& eventData)
{
    ...
    eastl::string type = eventData[P_TYPE].GetString();
...
void AudioManager::PlaySound(eastl::string filename, eastl::string type, int index)
{
...
SoundSource3D* AudioManager::CreateNodeSound(Node* node, const eastl::string& filename, const eastl::string& type)
{
    if (filename.empty())
    {
        return nullptr;
