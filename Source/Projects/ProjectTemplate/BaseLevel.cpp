...
void BaseLevel::SubscribeToBaseEvents()
{
    ...
    SubscribeToEvent("clip",
                     [&](StringHash eventType, VariantMap& eventData)
                     {
                         ...
                         if (params.size() == 3)
                         {
                             float near_ = ToFloat(params[1]);
                             float far_ = ToFloat(params[2]);
                             for (int i = 0; i < cameras_.size(); i++)
                             {
                                 cameras_[i]->GetComponent<Camera>()->SetNearClip(near_);
                                 cameras_[i]->GetComponent<Camera>()->SetFarClip(far_);
                                 URHO3D_LOGINFOF("Updating camera {%d}, near={%f}, far={%f}", i, near_, far_);
                             }
                         }
                     });
    ...
    SubscribeToEvent(E_SCREENMODE,
                     [&](StringHash eventType, VariantMap& eventData) { SendEvent(E_VIDEO_SETTINGS_CHANGED); });
    ...
    SubscribeToEvent(E_VIDEO_SETTINGS_CHANGED,
                     [&](StringHash eventType, VariantMap& eventData)
                     {
                         ...
                         for (int i = 0; i < GetSubsystem<Renderer>()->GetNumViewports(); i++)
                         {
                             ...
                             effectRenderPath->SetShaderParameter("ScreenHeight",
                                                                  GetSubsystem<Graphics>()->GetHeight());
 ...
 */
eastl::vector<IntRect> BaseLevel::InitRects(int count)
{
    auto* graphics = GetSubsystem<Graphics>();
    eastl::vector<IntRect> rects;
    if (count == 1)
    {
        // whole screen
 ...
 */
void BaseLevel::InitViewports(eastl::vector<int> playerIndexes)
{
...
void BaseLevel::CreateSingleCamera(int index, int totalCount, int controllerIndex)
{
    ...
    eastl::vector<IntRect> rects = InitRects(totalCount);
...
void BaseLevel::ApplyPostProcessEffects()
{
    ...
    for (int i = 0; i < GetSubsystem<Renderer>()->GetNumViewports(); i++)
    {
        ...
        effectRenderPath->SetShaderParameter(
            "AutoExposureAdaptRate",
        ...
        effectRenderPath->SetEnabled("BloomHDR",
                                     GetSubsystem<ConfigManager>()->GetBool("postprocess", "BloomHDR", false));
