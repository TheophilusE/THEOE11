...
void BehaviourTree::Init(const eastl::string& config)
{
    LoadConfig(config);
    SubscribeToEvents();
}

void BehaviourTree::LoadConfig(const eastl::string& config)
{
...
void BehaviourTree::SubscribeToEvents()
{
    ...
    SubscribeToEvent("#behaviour_debug",
                     [&](StringHash eventType, VariantMap& eventData) { controls_.Set(CTRL_FORWARD, true); });
}
