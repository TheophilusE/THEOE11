...
private:
    ...
    bool RaycastFromCamera(Camera* camera, float maxDistance, Vector3& hitPos, Vector3& hitNormal,
                           Drawable*& hitDrawable);

    void ShowPauseMenu();
    void PauseMenuHidden();

    SharedPtr<Player> CreatePlayer(int controllerId, bool controllable, const eastl::string& name = "",
                                   int nodeID = -1);
    ...
    eastl::hash_map<int, SharedPtr<Player>> players_;
    eastl::hash_map<Connection*, SharedPtr<Player>> remotePlayers_;
