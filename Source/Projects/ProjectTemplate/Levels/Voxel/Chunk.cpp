...
void Chunk::Init(Scene* scene, const Vector3& position)
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "chunk_rerender", ConsoleCommandAdd::P_EVENT,
              "#chunk_rerender", ConsoleCommandAdd::P_DESCRIPTION, "Rerender all chunks",
...
void Chunk::Load()
{
    ...
    eastl::string filename =
        "World/chunk_" + String(position.x_) + "_" + String(position.y_) + "_" + String(position.z_) + ".json";
...
//    shouldRender_ = true;
//    renderIndex_ = 0;
////    URHO3D_LOGINFO("Chunk " + String(position_) + " geometry calculated in " + String(loadTime.GetMSec(false)) +
///"ms");
//}
...
void Chunk::SetBlockData(const IntVector3& blockPosition, BlockType type)
{
    ...
    {
        SetTorchlight(blockPosition.x_, blockPosition.y_, blockPosition.z_, 0);
        GetSubsystem<LightManager>()->AddLightRemovalNode(blockPosition.x_, blockPosition.y_, blockPosition.z_,
                                                          lightLevel, this);
    }
    for (int i = 0; i < 6; i++)
    {
...
void Chunk::HandleAdd(StringHash eventType, VariantMap& eventData)
{
    ...
    if (data_[blockPosition.x_][blockPosition.y_][blockPosition.z_].type == BlockType::BT_AIR)
    {
        if (eventData[P_ACTION_ID].GetInt() == CTRL_DETECT)
        {
            ...
            //            + " Torch light: " + String(GetTorchlight(blockPosition.x_, blockPosition.y_,
            //            blockPosition.z_)) +
