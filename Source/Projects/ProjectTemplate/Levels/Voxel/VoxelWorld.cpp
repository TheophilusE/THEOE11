...
void VoxelWorld::Init()
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "chunk_visible_distance", ConsoleCommandAdd::P_EVENT,
              "#chunk_visible_distance", ConsoleCommandAdd::P_DESCRIPTION,
              "How far away the generated chunks are visible", ConsoleCommandAdd::P_OVERWRITE, true);
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "world_reset", ConsoleCommandAdd::P_EVENT,
              "#world_reset", ConsoleCommandAdd::P_DESCRIPTION, "Remove saved chunks", ConsoleCommandAdd::P_OVERWRITE,
              true);
...
void VoxelWorld::LoadChunk(const Vector3& position)
{
    ...
    //            * x); terrain.y_ = GetSubsystem<ChunkGenerator>()->GetTerrainHeight(terrain); positions.Push(terrain);
    ...
    //
    //    positions.Push(Vector3(fixedChunkPosition + Vector3::UP * SIZE_Y + Vector3::BACK * SIZE_Z + Vector3::LEFT *
    //    SIZE_X)); positions.Push(Vector3(fixedChunkPosition + Vector3::UP * SIZE_Y + Vector3::BACK * SIZE_Z +
    //    Vector3::RIGHT * SIZE_X)); positions.Push(Vector3(fixedChunkPosition + Vector3::UP * SIZE_Y + Vector3::FORWARD
    //    * SIZE_Z + Vector3::LEFT * SIZE_X)); positions.Push(Vector3(fixedChunkPosition + Vector3::UP * SIZE_Y +
    //    Vector3::FORWARD * SIZE_Z + Vector3::RIGHT * SIZE_X));
    //
    //    // Down
    ...
    //
    //    positions.Push(Vector3(fixedChunkPosition + Vector3::DOWN * SIZE_Y + Vector3::BACK * SIZE_Z + Vector3::LEFT *
    //    SIZE_X)); positions.Push(Vector3(fixedChunkPosition + Vector3::DOWN * SIZE_Y + Vector3::BACK * SIZE_Z +
    //    Vector3::RIGHT * SIZE_X)); positions.Push(Vector3(fixedChunkPosition + Vector3::DOWN * SIZE_Y +
    //    Vector3::FORWARD * SIZE_Z + Vector3::LEFT * SIZE_X)); positions.Push(Vector3(fixedChunkPosition +
    //    Vector3::DOWN * SIZE_Y + Vector3::FORWARD * SIZE_Z + Vector3::RIGHT * SIZE_X));
    //
...
void VoxelWorld::RemoveBlockAtPosition(const Vector3& position) { removeBlocks_.Push(position); }
...
bool VoxelWorld::ProcessQueue()
{
    ...
    for (auto it = observers_.Begin(); it != observers_.End(); ++it)
    {
        ...
        if (haveChanges)
        {
            ...
            URHO3D_LOGINFOF("Player %s moved to chunk %dx%dx%d", (*it)->GetName().CString(),
                            (int)currentChunkPosition.x_, (int)currentChunkPosition.y_, (int)currentChunkPosition.z_);
        }
    }
