...
void Loading::UpdateStatusMesage()
{
    ...
    {
        status_->SetText(fmt::format("{}", (int)(progress * 100)) +
                         "% "
                         "" +
                         statusMessage_ + "...");
    }
...
void Loading::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (!GetSubsystem<Engine>()->IsHeadless())
    {
        ...
        if (loadingBar_)
        {
            loadingBar_->SetWidth(progress *
                                  (GetSubsystem<Graphics>()->GetWidth() / GetSubsystem<UI>()->GetScale() - 20));
        }
    }
    if (progress >= 1.0f)
    {
...
void Loading::HandleServerConnected(StringHash eventType, VariantMap& eventData)
{
    ...
    SendEvent(E_LOADING_STEP_PROGRESS, LoadingStepProgress::P_EVENT, "ConnectServer", LoadingStepProgress::P_PROGRESS,
              0.5f);
}
