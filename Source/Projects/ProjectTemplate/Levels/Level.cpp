...
void Level::Init()
{
    ...
    eastl::vector<int> controlIndexes = controllerInput->GetControlIndexes();
    ...
    if (!GetSubsystem<Engine>()->IsHeadless())
    {
        for (auto it = controlIndexes.begin(); it != controlIndexes.end(); ++it)
        {
            ...
            {
                players_[(*it)] = CreatePlayer((*it), true, "Player " + fmt::format("{}", (*it)));
            }
        }
    }
    ...
    if (!GetSubsystem<Network>()->GetServerConnection())
    {
        for (int i = 0; i < 0; i++)
        {
            players_[100 + i] = CreatePlayer(100 + i, false, "Bot " + fmt::format("{}", 100 + i));
...
SharedPtr<Player> Level::CreatePlayer(int controllerId, bool controllable, const eastl::string& name, int nodeID)
{
...
void Level::HandleControllerConnected(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        players_[controllerIndex] =
            CreatePlayer(controllerIndex, true, "Player " + fmt::format("{}", controllerIndex + 1));
    }
}
    ...
    for (auto it = players_.begin(); it != players_.end(); ++it)
    {
        ...
        if (cameras_[playerId])
        {
            ...
            if (target)
            {
                ...
                cameras_[playerId]->SetPosition(target->GetPosition() + cameras_[playerId]->GetRotation() *
                                                                            Vector3::BACK *
...
{
    eastl::string name = eventData["Name"].GetString();
    if (name == "PauseWindow")
    {
        PauseMenuHidden();
    }
...
void Level::HandleClientConnected(StringHash eventType, VariantMap& eventData)
{
    ...
    remotePlayers_[newConnection] = CreatePlayer(REMOTE_PLAYER_ID, true, "Remote " + fmt::format("{}", REMOTE_PLAYER_ID));
...
bool Level::RaycastFromCamera(Camera* camera, float maxDistance, Vector3& hitPos, Vector3& hitNormal,
                              Drawable*& hitDrawable)
{
    ...
    eastl::vector<RayQueryResult> results;
