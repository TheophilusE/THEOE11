...
extern "C"
{
    ...
    JNIEXPORT void JNICALL Java_org_libsdl_app_SDLActivity_SendServiceCommand(JNIEnv* env, jobject obj, jint cmd,
...
void ServiceCmd::ReceiveCmdMessage(int cmd, int status, const char* message)
{
    ...
    messageData.message = message ? fmt::format("{}", message) : "";
}
...
void ServiceCmd::SendResponseMsg(const MessageData& msg)
{
    ...
    eventData["Message"] = "Got cmd: " + fmt::format("{}", msg.command) + "; Status: " + fmt::format("{}", msg.status) +
                           "; Msg: " + msg.message;
