...
void Player::RegisterConsoleCommands()
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "camera_distance", ConsoleCommandAdd::P_EVENT,
              ...
              "Camera distance from player (0 - 1st person view, > 0  - 3rd person view)",
...
void Player::CreateNode(Scene* scene, int controllerId, Terrain* terrain, int type)
{
    ...
    node_ = scene->CreateChild("Player" + fmt::format("{}", controllerId));
    ...
    if (type == 0)
    {
        ...
        rigidBody_->SetCollisionLayerAndMask(COLLISION_MASK_PLAYER | COLLISION_MASK_CHUNK_LOADER,
                                             COLLISION_MASK_PLAYER | COLLISION_MASK_CHECKPOINT |
                                                 COLLISION_MASK_OBSTACLES | COLLISION_MASK_GROUND |
...
void Player::HandlePhysicsPrestep(StringHash eventType, VariantMap& eventData)
{
    ...
    if (positionUI_)
    {
        ...
        eastl::string content = "X:" + fmt::format("{}", static_cast<int>(position.x_)) +
                                " Y:" + fmt::format("{}", static_cast<int>(position.y_)) +
                                " Z:" + fmt::format("{}", static_cast<int>(position.z_));
...
void Player::SetName(const eastl::string& name)
{
    if (node_)
    {
...
void Player::HandlePredictPlayerPosition(StringHash eventType, VariantMap& eventData)
{
    ...
    eastl::string name = eventData[P_NAME].GetString();
