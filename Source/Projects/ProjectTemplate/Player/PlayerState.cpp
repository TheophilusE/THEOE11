...
PlayerState::~PlayerState()
{
    ...
    players.erase(fmt::format("{}", GetPlayerID()));
...
void PlayerState::RegisterObject(Context* context)
{
    ...
    URHO3D_ACCESSOR_ATTRIBUTE("Name", GetPlayerName, SetPlayerName, eastl::string, "", AM_DEFAULT);
}
...
void PlayerState::AddScore(int value)
{
    ...
    if (value < 0)
    {
        ...
        notificationData["Message"] = name_ + " lost " + fmt::format("{}", -value) + " points";
    }
    else
    {
        notificationData["Message"] = name_ + " got " + fmt::format("{}", value) + " points";
    }
...
void PlayerState::OnScoreChanged()
{
    ...
    {
        VariantMap players = GetGlobalVar("Players").GetVariantMap();
        VariantMap playerData = players[fmt::format("{}", GetPlayerID())].GetVariantMap();
        playerData["Score"] = score_;
        playerData["ID"] = GetPlayerID();
        playerData["Name"] = GetPlayerName();
        players[fmt::format("{}", GetPlayerID())] = playerData;
        SetGlobalVar("Players", players);
...
void PlayerState::SetPlayerName(const eastl::string& name)
{
    name_ = name;
    OnScoreChanged();
    MarkNetworkUpdate();
...
const eastl::string& PlayerState::GetPlayerName() const { return name_; }
