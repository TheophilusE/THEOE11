...
void ControllerInput::LoadConfig()
{
    ...
    {
        eastl::string controlName = (*it).second;
...
void ControllerInput::SaveConfig()
{
    ...
    for (auto it = inputHandlers_.begin(); it != inputHandlers_.end(); ++it)
    {
        eastl::hash_map<int, int> configMap = (*it).second->GetConfigMap();
        int type = (*it).first;
        eastl::string typeName;
        eastl::hash_map<int, eastl::string> map;
        ...
        for (auto it2 = configMap.begin(); it2 != configMap.end(); ++it2)
        {
            ...
            {
                eastl::string controlName = controlMapNames_[controlCode];
                controlName.replace(" ", "_");
                eastl::string value = fmt::format("{}", keyCode);
...
void ControllerInput::SetConfiguredKey(int action, int key, eastl::string controller)
{
...
void ControllerInput::HandleStartInputListening(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        eastl::string control = eventData[P_CONTROL_ACTION].GetString();
        for (auto it = controlMapNames_.begin(); it != controlMapNames_.end(); ++it)
        {
...
eastl::hash_map<int, eastl::string> ControllerInput::GetControlNames() { return controlMapNames_; }

eastl::string ControllerInput::GetActionKeyName(int action)
{
    ...
    for (auto it = inputHandlers_.begin(); it != inputHandlers_.end(); ++it)
    {
        ...
        {
            eastl::string keyName = (*it).second->GetActionKeyName(action);
            return keyName;
        }
    }
...
eastl::vector<int> ControllerInput::GetControlIndexes()
{
    eastl::vector<int> indexes;
    if (!multipleControllerSupport_)
    {
        indexes.push_back(0);
        return indexes;
