...
void SingleAchievement::SetImage(eastl::string image)
{
    auto* cache = GetSubsystem<ResourceCache>();
    sprite_->SetTexture(cache->GetResource<Texture2D>(image));
}

void SingleAchievement::SetMessage(eastl::string message)
{
    message_ = "";
    title_->SetText(message);
}

eastl::string SingleAchievement::GetMessage() { return message_; }
