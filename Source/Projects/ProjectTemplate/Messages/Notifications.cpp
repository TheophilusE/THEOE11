...
{
    eastl::string message = eventData["Message"].GetString();
    eastl::string status = eventData["Status"].GetString();
    ...
    if (timer_.GetMSec(false) < NOTIFICATION_OVERLAP_TIME)
    {
        ...
        URHO3D_LOGINFOF("Too many notification request, pushing notification on queue. Queue size {%d}",
                        messageQueue_.size());
        return;
    }
